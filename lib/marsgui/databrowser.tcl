...
#
# DESCRIPTION:
#    marsgui(n): data browser widget
#
#    This widget displays a tabular list of data records; it is intended
#    for the display of in-memory data.  The data source needs to 
#    provide the following:
#
#    * A command that returns a list of record IDs
#    * A command that returns a record's dictionary given an ID.
#
#    In the code, the record's ID is also known as its "unique ID" or
#    "uid".  It is not necessary that the uid appear in the record 
#    returned by the -dictcmd.
#
#    The name "_uid" is reserved by this widget; it may not appear 
#    as an attribute record name.
#
#-----------------------------------------------------------------------
...
snit::widget ::marsgui::databrowser {
    hulltype ttk::frame
    ...
    # -sourcecmd cmd
    #
    # A command that returns the IDs of the data records to display.
    # To display a different set of records, change this command.
    ...
    # -dictcmd cmd
    #
    # A command that returns a data record as a Tcl dictionary, given
    # its ID.  It receives one additional argument, the ID.  
    # Note that the record ID must appear in the dictionary with key
    # "id".
    ...
    # -layout spec
    #
    # A specification of the columns to display.
    #
    ...
    #    name label options...
    #
    # where column name is a record attribute name, label is the column
    # heading, and the options are a subset of the tablelist 
    # columnconfigure options.
    #
    # NOTE: a hidden column for the _uid will be added at the end.
    ...
    method ReloadContent {} {
        ...
        foreach uid [callwith $options(-sourcecmd)] {
            # FIRST, get the data list for this record, and insert it.
            set data [$self GetRecordAsList $uid]
    ...
    method GetRecordAsList {uid} {
        ...
        foreach name $info(columns) {
            lappend data [dict get $rdict $name]
        }
    ...
    method UpdateUidMap {} {
        ...
        set cindex [dict get $layout(_uid) cindex]
    ...
    method {uid curselection} {} {
        ...
        set cindex [$self cname2cindex _uid]
    ...
    # uid update uid
    #
    # uid    - A unique record ID
    #
    # A record that has already been displayed has been modified.  Update 
    # the relevant row.

    method {uid update} {uid} {
        # FIRST, this call is only for updating records that 
        # are already displayed in this browser.  If no such 
        # record is displayed, we can ignore the call.
        if {![info exists uidmap($uid)]} {
            return
        }
        ...
        # NEXT, get the record data and prepare it to be loaded into the
        # tablelist.
        set data [$self GetRecordAsList $uid]
    ...
    method {uid delete} {uid} {
        # FIRST, look for a match on uid.  If the record isn't currently
